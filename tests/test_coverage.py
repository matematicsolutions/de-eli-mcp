...
from de_eli_mcp.coverage import build_coverage
from de_eli_mcp.server import de_coverage


def _coverage():
    fn = getattr(de_coverage, "fn", de_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DE_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("de_coverage" in line for line in lines), (
        "an audit entry was written but it does not name de_coverage"
    )
